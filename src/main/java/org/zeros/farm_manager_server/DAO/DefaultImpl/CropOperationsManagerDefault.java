...
public class CropOperationsManagerDefault implements CropOperationsManager {
    ...
    public CropOperationsManagerDefault(CropRepository cropRepository, EntityManager entityManager, CultivationRepository cultivationRepository, SeedingRepository seedingRepository, SprayApplicationRepository sprayApplicationRepository, FertilizerApplicationRepository fertilizerApplicationRepository, HarvestRepository harvestRepository, CropSaleRepository cropSaleRepository, SubsideManager subsideManager, FarmingMachineManager farmingMachineManager, CropParametersManager cropParametersManager, SprayManager sprayManager, FertilizerManager fertilizerManager) {
    ...
    public void deleteCropAndItsData(@NonNull Crop crop) {
        /*cultivationRepository.deleteAll(crop.getCultivations());
        seedingRepository.deleteAll(crop.getSeeding());
        sprayApplicationRepository.deleteAll(crop.getSprayApplications());
        fertilizerApplicationRepository.deleteAll(crop.getFertilizerApplications());
        if (crop.getClass().equals(MainCrop.class)) {
            harvestRepository.deleteAll(((MainCrop) crop).getHarvest());
            cropSaleRepository.deleteAll(((MainCrop) crop).getCropSales());
        }*/
    ...
    @Override
    public Crop setWorkFinished(@NonNull MainCrop mainCrop) {
        mainCrop = (MainCrop) getCropById(mainCrop.getId());
        mainCrop.setWorkFinished(true);
        flushChanges();
        return getCropById(mainCrop.getId());
    }

    @Override
    public Crop setFullySold(@NonNull MainCrop mainCrop, @NonNull Boolean isFullySold) {
        mainCrop = (MainCrop) getCropById(mainCrop.getId());
    ...
    private AgriculturalOperation getOperationAccordingToType(AgriculturalOperation agriculturalOperation) {
