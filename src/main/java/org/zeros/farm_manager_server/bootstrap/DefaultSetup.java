...
import org.zeros.farm_manager_server.DAO.DefaultImpl.CropParametersManagerDefault;
...
public class DefaultSetup {
    ...
    public DefaultSetup(@Autowired FertilizerManager fertilizerManager, @Autowired SprayManager sprayManager, @Autowired FarmingMachineManager farmingMachineManager, @Autowired SubsideRepository subsideRepository, @Autowired SpeciesManager plantSpeciesManager, PlantManager plantManager, @Autowired SubsideManager subsideManager, SpeciesRepository speciesRepository, UserRepository userRepository, LoggedUserConfiguration loggedUserConfiguration, CropParametersRepository cropParametersRepository, CropParametersManager cropParametersManager) {
...
protected void createTestSpecies() {
        if (speciesManager.getDefaultSpecies(0).isEmpty()) {
            speciesManager.addSpecies(Species.builder().name("Corn").family("Grass family").build());
            speciesManager.addSpecies(Species.NONE);
            speciesManager.addSpecies(Species.ANY);
            speciesManager.addSpecies(Species.builder().name("Wheat").family("Grass family").build());
            speciesManager.addSpecies(Species.builder().name("Rape seed").family("Crucifer family").build());
        }
    ...
    private void createTestSubsides() {
        if (subsideManager.getDefaultSubsides(0).isEmpty()) {
            subsideManager.addSubside(Subside.builder().name("Field payment").speciesAllowed(Set.of(speciesRepository.getSpeciesByName("ANY").orElse(Species.NONE))).yearOfSubside(LocalDate.ofYearDay(2024, 22)).subsideValuePerAreaUnit(BigDecimal.valueOf(300)).build());
            subsideManager.addSubside(Subside.builder().name("Wheat subside").speciesAllowed(Set.of(speciesRepository.getSpeciesByName("Wheat").orElse(Species.NONE))).yearOfSubside(LocalDate.ofYearDay(2024, 22)).subsideValuePerAreaUnit(BigDecimal.valueOf(300)).build());

            subsideManager.addSubside(Subside.builder().name("Rape subside").speciesAllowed(Set.of(speciesRepository.getSpeciesByName("Rape seed").orElse(Species.NONE))).yearOfSubside(LocalDate.ofYearDay(2024, 22)).subsideValuePerAreaUnit(BigDecimal.valueOf(300)).build());
        }
    }
    ...
    private void createTestFarmingMachines() {
        if (farmingMachineManager.getDefaultFarmingMachines(0).isEmpty()) {
            ...
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("Cataya 3000").producer("Amazone").supportedOperationTypes(Set.of(OperationType.CULTIVATION, OperationType.SEEDING)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("Wicher 4500").producer("Unia").supportedOperationTypes(Set.of(OperationType.CULTIVATION)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("R1212").producer("Unia").supportedOperationTypes(Set.of(OperationType.FERTILIZER_APPLICATION)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("Precea 6").producer("Amazone").supportedOperationTypes(Set.of(OperationType.SEEDING)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("N 300").producer("AgroLift").supportedOperationTypes(Set.of(OperationType.CULTIVATION)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("AlN 27000").producer("Caruelle").supportedOperationTypes(Set.of(OperationType.SPRAY_APPLICATION)).build());
            farmingMachineManager.addFarmingMachine(FarmingMachine.builder().model("Trion 640").producer("Class").supportedOperationTypes(Set.of(OperationType.HARVEST)).build());
    ...
    private void createTestFertilizers() {
        if (fertilizerManager.getDefaultFertilizers(0).isEmpty()) {
            ...
            fertilizerManager.addFertilizer(Fertilizer.builder().name("Polifoska 6").producer("Azoty").isNaturalFertilizer(false).N_Percent(BigDecimal.valueOf(6)).P_Percent(BigDecimal.valueOf(20)).K_Percent(BigDecimal.valueOf(30)).build());
            fertilizerManager.addFertilizer(Fertilizer.builder().name("Polifoska 8").producer("Azoty").isNaturalFertilizer(false).N_Percent(BigDecimal.valueOf(8)).P_Percent(BigDecimal.valueOf(20)).K_Percent(BigDecimal.valueOf(30)).build());
            fertilizerManager.addFertilizer(Fertilizer.builder().name("Polifoska 16").producer("Azoty").isNaturalFertilizer(false).N_Percent(BigDecimal.valueOf(16)).P_Percent(BigDecimal.valueOf(20)).K_Percent(BigDecimal.valueOf(30)).build());
        }
    }
